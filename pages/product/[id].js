...
export default function ProductDetails({ item }) {
  ...
  return (
    <div className="bg-rgba(0, 0, 0, 0) w-full">
      ...
      <div className="container px-5 py-24 mx-auto cursor-auto">
        <div className="lg:w-4/5 mx-auto flex flex-wrap">
          <img
            alt="item.name"
            className="lg:w-1/2 w-full lg:h-1/2 h-64 object-cover object-center rounded cursor-auto"
            src={item?.image}
          />
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0 cursor-auto">
            <h2 className="text-sm title-font text-gray-500 tracking-widest cursor-auto">
              ON SALE
            </h2>
...
ProductDetails.getInitialProps = async ({ query }) => {
  const data = fetch(`http://localhost:3000/products?id=${query.id}`)
    .then((res) => res.json())
    .then((result) => {
      return result;
    })
